...
Paste or type JSON into the box and a tree appears below it. Objects and arrays start collapsed; a chevron opens each one, and large containers open 200 children at a time so a multi-megabyte document does not freeze the page. Every row has a copy button that puts that node's JSONPath on the clipboard, using dot notation for plain keys and quoted brackets for anything else, so `$.items[0]['user name']` comes out correct rather than merely plausible. Bad input gets one plain-language line under the box with the line and column, and the page keeps working. The screenshot shows the sample document expanded two levels, with the copy button on the `user name` row reading `copied` just after it handed over `$.items[0]['user name']`.
